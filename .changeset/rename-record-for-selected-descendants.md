...
Inheritance stops at any node carrying provenance of its own — a detached component's record as much as a pattern's, because its subtree did not come from the host either. A reference is restored only where the node holding it is governed by the record naming it: a node moved out of the run keeps its reference, and rewriting that would point it somewhere the saved forest never had.

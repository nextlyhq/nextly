...
A restore is applied only where EVERY node referencing the id gives the same answer, so one governed reference cannot rewrite an unrelated author's; and a record storage would not keep — a non-enumerable `origin`, which JSON, an object spread and `structuredClone` all drop — is not read for its contents, though it still bounds the scope.

...
The shared node walk also stops crashing on a node whose `slots` refuse to be read, which is the same tolerance it already had for a slot holding something other than a list. A reference is restored only where the node holding it is governed by the record naming it: a node moved out of the run keeps its reference, and rewriting that would point it somewhere the saved forest never had.

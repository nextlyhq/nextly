...
A block can declare the parents it may sit under — `parent` on `BlockDefinition`, matching the field of the same name in Gutenberg's block metadata — enforced on the editor and the write path alike, with the repair banner offering to wrap a stray block in the parent it names. This is the half a slot's `allowedBlocks` cannot express, and it is available to plugin blocks as well as core ones: `core/column` uses it so inserting a Column while one is selected produces a sibling in the row rather than a column nested inside a column.

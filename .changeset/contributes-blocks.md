...
`defineBlock` and the block types are re-exported from `@nextlyhq/plugin-page-builder`, so a block author takes one package. Nextly core is unchanged: it carries no blocks contribution key and does not depend on the block engine, because contributing blocks is contributing to the page builder rather than to the framework.

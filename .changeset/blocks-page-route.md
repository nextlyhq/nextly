...
It also now **refuses a slug holding a `.` or `..` segment**, including the percent-encoded spellings. URL resolution removes those segments before the request is sent, so a page pre-rendered at `/pages/../admin` is fetched as `/admin` and can never be reached, while occupying a path that belongs to a different and possibly reserved route. Percent-encoding does not help: the URL standard counts `%2e` as a dot for exactly this purpose. Only a segment that is entirely dots is affected — `docs/v1.2/guide` is an ordinary path.

...
- A Schema Builder save now writes the SQL it applied into the dynamic migrations folder — including renames and drops, not just added columns — so a change made visually can be reproduced on a fresh database instead of existing only in the live one.

...
`addressableFields` is published from `nextly` and re-exported from `@nextlyhq/plugin-sdk` as `@experimental`, recorded in that package's stability ledger. The page-builder's own copy still stands: the two walks disagree about whether to descend an unnamed field that is not a group, and reconciling that is a behavioural decision for the lane that owns it. It had no tests anywhere; it has fifteen now, eight of which pin what it already did so the move can be shown to have changed nothing that was working.

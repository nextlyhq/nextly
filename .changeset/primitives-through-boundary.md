...
A candidate URL clears BOTH filters before the block chooses between them, and a media record whose URL either filter refuses is dropped whole. The two refuse different things — the scheme guard refuses a value that could execute, the host list refuses one the site will not fetch from — and this block has now been caught twice applying one of them at one position of the resolver/typed-prop pair and not the other. The same pair reaches the link preview, so the scheme guard runs there too: with no host list configured it is the only thing between a media record and the tag every crawler unfurls, and the preview must not advertise an image the page refuses to draw.

A transparent wrapper's props are read BY NAME rather than by enumerating them. Enumeration answers a different question than React asks: it reports enumerable own properties, while React simply reads the property. A forged element carrying a throwing `value` getter as non-enumerable, or inheriting one, is invisible to enumeration and not to React, so the exception escaped the boundary and cost the page.

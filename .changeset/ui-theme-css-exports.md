...
- `@nextlyhq/ui/theme.css` — the raw design-system source (token definitions on `:root`/`.dark`, the `@theme inline` mappings, and the `dark` variant) for consumers that compile Tailwind themselves and want to build against the token contract.

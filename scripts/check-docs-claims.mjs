...
function readableProse(markdown, { mdx }) {
  ...
  const drawn = markdown.replace(
    ...
    fence =>
      // A `click` binds a node to an address, so its string is a target rather
      // than something drawn.
      (fence.replace(/^[ \t]*click\b[^\n]*$/gm, "").match(/"[^"\n]*"|'[^'\n]*'|\[[^\]\n]*\]|\([^)\n]*\)|\|[^|\n]*\||\{[^}\n]*\}/g) ?? [])
 ...
 * without this the two would read as one sentence. The other direction is not a
 * boundary: an unprefixed line beneath a quote line continues it.
 */
function splitOnQuoteBoundary(block) {
  ...
  for (const line of block.split("\n")) {
    ...
    // Only entering a quotation is a boundary. An unprefixed line beneath one is
    // a lazy continuation of the same paragraph rather than the end of it, and a
    // blank line has already closed the block by the time this runs.
    if (quoted === false && isQuoted && current.length > 0) {

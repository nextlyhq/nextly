...
describe("a message's credit, in each form", () => {
  ...
  it("accepts a mention that credits nothing", () => {
    const mentions = [
      ...
      "Written by Claude Code researcher Jane Doe",
      "Thanks to the Mistral Large team",
      "Written by Mistral Large researcher Jane",
      "Built with the Copilot Chat SDK",
...
describe("a line a change adds", () => {
  ...
  it("passes a page or a document a product makes with AI, and refuses only this file or this change so described", () => {

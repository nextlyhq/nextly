...
describe("the command in CI", () => {
  ...
  // A plain line under a vendor is no name in progress, so it does not join the tool named after it into one name that credits no one.
  it("refuses a tool added after a plain line under a vendor's name that was already there", () => {

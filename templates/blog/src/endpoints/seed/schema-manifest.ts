 ...
 * `src/collections/*` and `src/singles/*` declare via `defineCollection`
...
export interface ManifestCollection {
  ...
  fields: ManifestField[];
  /**
   * Whether the collection has Nextly's built-in Draft/Published
   * lifecycle. When true, Phase A registers the collection with
   * `status: true` and the schema pipeline injects a NOT NULL `status`
   * system column (default 'draft'). Mirrors the `status` flag accepted
   * by `defineCollection` in the code-first approach.
   */
  status?: boolean;
}
  ...
  collections: [
    {
      ...
      // Why: enable Nextly's built-in Draft/Published lifecycle. Mirrors
      // `status: true` on the code-first Posts.defineCollection so visual
      // and code-first scaffolds converge on the same DB shape.
      status: true,
      fields: [

...
- Newsletter signup via the `@nextlyhq/plugin-form-builder` plugin
...
- **Posts** - Title, slug, rich text content, featured image, author (relates to users), categories, tags, excerpt, publish date, featured flag, SEO group (metaTitle, metaDescription, ogImage, canonical, noindex). Auto-generates slug from title; computes reading time and word count on save. Uses Nextly's built-in **Draft/Published** lifecycle (`status: true` on the collection) so the admin shows separate Save Draft / Publish buttons and the public REST endpoint hides drafts by default.
...
The homepage and footer newsletter forms submit to the `@nextlyhq/plugin-form-builder` plugin. Submissions are stored in the `form-submissions` collection and visible at `/admin/collections/form-submissions`.
...
templates/blog/
  ...
  src/
    ...
    lib/
      ...
      site-url.ts
      __scratch__/         # Developer scratchpad — debug-queries etc., not for prod

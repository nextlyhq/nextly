...
describe("measureBytes agrees with the serializer on hooks and cycles", () => {
  it("refuses a cyclic value in exact-count mode instead of hanging", () => {
    ...
    //
    // Reported as UNSTORABLE rather than as oversized, which are different
    // facts about the value and now have different fields. A cycle has no
    // serialized form at any size, so folding it into `exceeded` made every
    // caller's message wrong in the same way — telling an author that a
    // two-node document had outgrown its byte cap. `exceeded` answers only the
    // size question; a caller deciding "can I store this" reads both.

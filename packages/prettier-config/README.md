...
```json
{
  "prettier": "@nextlyhq/prettier-config"
}
```

## Used by

The repo-level `lint-staged` config and any workspace package that opts in via `prettier: "@nextlyhq/prettier-config"` in `package.json`.

...
export async function createAdapter(
  ...
  // ("await import(moduleId)") to prevent bundlers from pulling in every
  // adapter at compile time, but Turbopack and modern webpack can't
  // resolve a runtime-variable specifier and fail at "Cannot find module"
  // even when the package is installed. With literal strings each branch
  // is statically analysable; bundlers only emit code for branches that
  // are actually reachable, so the size cost is marginal (~30 KB per
  // dialect, server-only — clients never download adapter code).
  //
  // Each branch creates and connects its adapter inline. Drives type
  // inference end-to-end without forcing `unknown` casts: tsc sees the
  // real PostgresAdapter / MySqlAdapter / SqliteAdapter return types from
  // each adapter package.
  // Validate dialect before any import. Once we get past this guard the
  // remaining branches are exhaustive over `"postgresql" | "mysql" | "sqlite"`,
  // which is what lets us drop the `as unknown as ...` casts below.

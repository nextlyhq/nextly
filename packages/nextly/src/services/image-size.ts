...
export class ImageSizeService extends BaseService {
  ...
  async getActiveSizeConfigs(): Promise<ImageSizeConfig[]> {
    ...
    return (rows as ImageSize[]).map(row => ({
      name: row.name,
  ...
  async create(input: CreateImageSizeInput): Promise<ImageSize> {
    ...
    const data = {
      ...
      // isDefault means "code-defined" (not "the default for picker thumbnails").
      // Default to false so non-sync callers don't accidentally mark an entry
      // as code-defined; the syncImageSizes pathway sets true explicitly.

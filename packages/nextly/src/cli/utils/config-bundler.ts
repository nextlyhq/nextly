...
//   Workspace packages (@nextlyhq/*) are published as ESM-only — their
//   exports maps have only an "import" condition, no "require". When the
//   config file imports e.g. `nextly/config`, those imports are
...
export async function bundleAndRequire(
  ...
  // Without this, `nextly/config` triggers esbuild to bundle

...
describe("reloadNextlyConfig", () => {
  ...
  describe("component table targets on reload", () => {
    it("diffs the table the registry records, not the configured dbName", async () => {
      // The registry retains the populated table when a component's configured
      // dbName changes; the HMR diff must follow it, or DDL lands on a table the
      // runtime never reads.
      loadConfigSpy.mockResolvedValue({
        config: {
          components: [
      ...
      introspectSpy.mockResolvedValue(
        buildSnapshot([{ name: "seo_meta", columns: SQLITE_RESERVED }])
      );

      const { reloadNextlyConfig } = await import("../reload-config");
      await reloadNextlyConfig({
        resolver: buildResolver({
          allComponents: [{ slug: "seo", tableName: "seo_meta", fields: [] }],
        }),
      });

      const args = introspectSpy.mock.calls[0] as [unknown, string, string[]];
      expect(args[2]).toContain("seo_meta");
      expect(args[2]).not.toContain("seo_v2");
    });

    it("falls back to the configured name for a component the registry has not seen", async () => {
      loadConfigSpy.mockResolvedValue({
        config: {
          components: [
            {
      ...
      introspectSpy.mockResolvedValue(
        buildSnapshot([{ name: "hero_blocks", columns: SQLITE_RESERVED }])
      );

      const { reloadNextlyConfig } = await import("../reload-config");
      await reloadNextlyConfig({
        resolver: buildResolver({ allComponents: [] }),
      });

      const args = introspectSpy.mock.calls[0] as [unknown, string, string[]];
      expect(args[2]).toContain("hero_blocks");
    });
  });
});

 ...
 * EmailSchema (RFC 5321 + trim/lowercase) and PasswordSchema (length +
 * class requirements) so register matches the checks the rest of the auth
 * surface already enforces. Name is bounded to a sensible UI range; not
 * load-bearing security, but rejects trivially silly inputs (empty string,
...
export interface RegisterHandlerDeps {
  ...
  /**
   * Minimum response time in milliseconds. The handler stalls every
   * response path to this floor so an attacker cannot distinguish
   * "fresh account created" (slow; bcrypt + DB inserts) from "conflict
   * swallowed" (fast; just a DB read) by timing.
   */

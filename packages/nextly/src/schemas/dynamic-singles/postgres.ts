/**
 * PostgreSQL Schema for Dynamic Singles
 *
 ...
 *
 * @module schemas/dynamic-singles/postgres
 * @since 1.0.0
 *
 * @example
 * ```typescript
 * import {
 *   dynamicSinglesPg,
 *   type DynamicSinglePg,
 *   type DynamicSingleInsertPg,
 * } from '@nextly/schemas/dynamic-singles/postgres';
 *
 * // Insert a new Single
 * const newSingle = await db.insert(dynamicSinglesPg).values({
 *   slug: 'site-settings',
 *   label: 'Site Settings',
...
// ============================================================
// Dynamic Singles Table (PostgreSQL)
// ============================================================

/**
 * PostgreSQL schema for the `dynamic_singles` table.
 *
 ...
 *   .select()
 *   .from(dynamicSinglesPg)
 *   .where(eq(dynamicSinglesPg.source, 'code'));
 *
 * // Find Singles needing migration
 * const pendingMigrations = await db
 *   .select()
 *   .from(dynamicSinglesPg)
 *   .where(eq(dynamicSinglesPg.migrationStatus, 'pending'));
 * ```
 */
export const dynamicSinglesPg = pgTable(
  "dynamic_singles",
  {
    // --------------------------------------------------------
    // Primary Key
     ...
     */
    admin: jsonb("admin").$type<SingleAdminOptions>(),
     ...
     */
    accessRules: jsonb("access_rules").$type<SingleAccessRules>(),
...
/**
 * PostgreSQL-specific select type for dynamic Singles.
 *
 ...
 * @example
 * ```typescript
 * const single: DynamicSinglePg = await db
 *   .select()
 *   .from(dynamicSinglesPg)
 *   .where(eq(dynamicSinglesPg.slug, 'site-settings'))
 *   .limit(1)
 *   .then(rows => rows[0]);
 * ```
 */
export type DynamicSinglePg = typeof dynamicSinglesPg.$inferSelect;

/**
 * PostgreSQL-specific insert type for dynamic Singles.
 *
 ...
 * @example
 * ```typescript
 * const newSingle: DynamicSingleInsertPg = {
 *   slug: 'site-settings',
 *   label: 'Site Settings',
 *   tableName: 'single_site_settings',
 ...
 * };
 *
 * await db.insert(dynamicSinglesPg).values(newSingle);
 * ```
 */
export type DynamicSingleInsertPg = typeof dynamicSinglesPg.$inferInsert;

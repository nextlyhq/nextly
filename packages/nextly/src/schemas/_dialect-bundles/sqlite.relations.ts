/**
 * Drizzle v2 relations for the SQLite dialect bundle.
 *
 * One central `defineRelations` per dialect replaces the per-feature
 * `<feature>/sqlite-relations.ts` files (drizzle v1 removed the
 ...
 *
 * @module schemas/_dialect-bundles/sqlite.relations
 */

import { defineRelations } from "drizzle-orm";
import type { ExtractTablesFromSchema, RelationsBuilder } from "drizzle-orm";

import * as tables from "./sqlite";

type SqliteTables = ExtractTablesFromSchema<typeof tables>;
...
export const relations = defineRelations(tables, buildSqliteEdges);

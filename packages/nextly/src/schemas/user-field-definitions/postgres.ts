/**
 * PostgreSQL Schema for User Field Definitions
 *
 * Defines the `user_field_definitions` table schema for PostgreSQL databases
 ...
 *
 * @module schemas/user-field-definitions/postgres
 * @since 1.0.0
 *
 * @example
 * ```typescript
 ...
 * } from '../schemas/user-field-definitions/postgres';
 *
 * // Insert a new field definition
 * await db.insert(userFieldDefinitionsPg).values({
 *   name: 'company',
 *   label: 'Company',
 *   type: 'text',
 *   source: 'ui',
...
// ============================================================
// User Field Definitions Table (PostgreSQL)
// ============================================================

/**
 * PostgreSQL schema for the `user_field_definitions` table.
 *
 ...
 *   .select()
 *   .from(userFieldDefinitionsPg)
 *   .where(eq(userFieldDefinitionsPg.isActive, true))
 *   .orderBy(asc(userFieldDefinitionsPg.sortOrder));
 *
 * // Query code-sourced fields only
 * const codeFields = await db
 *   .select()
 *   .from(userFieldDefinitionsPg)
 *   .where(eq(userFieldDefinitionsPg.source, 'code'));
 * ```
 */
export const userFieldDefinitionsPg = pgTable(
  "user_field_definitions",
  {
     ...
     * its own declaration.
     */
    pluginOptions: jsonb("plugin_options").$type<Record<
      string,
      unknown
    > | null>(),
...
/**
 * PostgreSQL-specific select type for user field definitions.
 * Represents a full row from the `user_field_definitions` table.
 */
export type UserFieldDefinitionPg = typeof userFieldDefinitionsPg.$inferSelect;

/**
 * PostgreSQL-specific insert type for user field definitions.
 * Fields with defaults (id, required, sortOrder, source, isActive, timestamps) are optional.
 */
export type UserFieldDefinitionInsertPg =
  typeof userFieldDefinitionsPg.$inferInsert;

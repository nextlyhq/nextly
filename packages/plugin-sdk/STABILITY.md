...
| Plugin UI kit (`@nextlyhq/ui`)                                          | Shared React primitives consumed as a host **peer dep**: `Button`, `Input`, `Textarea`, `Checkbox`, `Label`, `RadioGroup`, `Switch`, `Select`, `Tabs`, `Tooltip`, `Dialog`, `Popover`, `DropdownMenu`, `Card`, `Table`, `FormLabelWithTooltip`, … (see `@nextlyhq/ui` exports). Plugins import UI primitives from here — **never** from `@nextlyhq/admin`. | form-builder (field editors)          | D68/D53     |
...
| Custom field types (D16/M9a)              | `contributes.fieldTypes`, `PluginFieldType`                                                                                                                                                                                                                                                    | New in Phase 3. **Storage-classifier seam only**: a custom type maps to a storage primitive (DDL). Config-validation acceptance + admin rendering are the M9 remainder. No first-party plugin yet. |

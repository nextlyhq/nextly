...
export const entryApi = {
  ...
  find: async (
    ...
    // this; that still narrows correctly because where-filtering happens
    // after the default-bypass.
    const search = new URLSearchParams(buildFindQuery(params));
    if (!search.has("status")) search.set("status", "all");
  ...
  findByID: async (
    ...
    // demotes a published entry and immediately re-renders the page.
    // Public REST callers omit this param and continue to get the safe
  ...
  count: async (
    ...
    // only total, which would mismatch the list view's row count and
    // confuse pagination.
    query.set("status", "all");

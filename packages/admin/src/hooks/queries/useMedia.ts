...
export function useMoveMediaToFolder() {
  ...
  >({
    ...
    onSuccess: () => {
      // Why invalidate, not remove: the MoveToFolderDialog runs a
      // Promise.allSettled over multiple per-item mutations. The first
      // mutation's onSuccess fires this hook synchronously, and a
      // `removeQueries` here used to wipe the parent MediaLibrary's
      // cached data mid-flight, causing it to re-render with `undefined`
      // and orphan the in-flight per-call onSuccess callbacks of the
      // remaining mutations. The user-visible symptom: the move
      // persisted server-side, but the dialog never closed and an
      // error toast surfaced. invalidateQueries marks the data stale
      // and refetches in the background without dropping the cache,
      // so the dialog and bulk action bar render stably until the

...
export function useEntryJSON({
  ...
  // Build the API URL for display + the "Open" link. Two corrections from
  //
  // 1. depth must always be sent. The previous `depth > 0 ?` gate omitted
  //    the param when the user picked 0, so the server fell back to its
  //    DEFAULT_RELATIONSHIP_DEPTH (=2) and expanded relations anyway,
  //    making the depth=0 setting silently ineffective.
  //
  // 2. The route is mounted at /admin/api/* (Next.js catch-all in
  //    apps/playground/src/app/admin/api/[[...params]]/route.ts and the
  //    same convention in user apps). The previous /api/* prefix had no
  //    handler, so the dialog's "Open" button always 404'd. APIPlayground
  //    and other admin services already use /admin/api/* — this brings
  //    the dialog in line.
  const depthSuffix = `?depth=${depth}`;
  const apiUrl =
    scope === "single"
      ? `/admin/api/singles/${collectionSlug}${depthSuffix}`
      : `/admin/api/collections/${collectionSlug}/entries/${entryId}${depthSuffix}`;

...
export function safeHref(href: string): boolean {
  // The characters a browser discards while parsing, discarded first, so the
  // scheme judged here is the scheme the browser would see. Whitespace and
  // the control category together cover more than a browser strips, and the
  // excess can only REFUSE an href a browser would have read as a harmless
  // relative path -- never admit one it would have run.
  const cleaned = href.replace(/[\s\p{Cc}]+/gu, "");

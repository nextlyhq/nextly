...
export function GroupInput({
  ...
  // =========================================
  // Why: when `admin.hideGutter: true`, drop the Card chrome (border,
  // padded background, indented content) and render the group as a flat
  // collapsible — label + chevron above, fields directly below — so a
  // group declared purely for nesting field paths doesn't add visual
  // weight. The previous renderer ignored the knob entirely and always
  // wrapped in a Card.
  const hideGutter = field.admin?.hideGutter === true;
  ...
  if (hideGutter) {
    return (
      <Collapsible
        ...
        className={cn(field.admin?.className)}
      >
        {field.label && (
        ...
        {field.label && field.admin?.description && isOpen && (

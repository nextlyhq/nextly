"use client";

import type { FieldConfig } from "@revnixhq/nextly/config";
import { ChevronDown } from "lucide-react";
import type React from "react";
import { useState } from "react";

import { EntryFormContent } from "@admin/components/features/entries/EntryForm/EntryFormContent";
import { useAdminDateFormatter } from "@admin/hooks/useAdminDateFormatter";
import { cn } from "@admin/lib/utils";

import type { SingleDocumentData, SingleSchema } from "./SingleForm";

// ============================================================================
// Types
// ============================================================================

export interface SingleFormSidebarProps {
  /** The Single document data */
  document: SingleDocumentData;
  /** The Single schema */
  schema: SingleSchema;
  /** Explicitly extracted seo field group */
  seoField?: FieldConfig;
  /** Action buttons */
  actions?: React.ReactNode;
  /** Sidebar-positioned fields (admin.position === 'sidebar') */
  fields?: FieldConfig[];
}

// ============================================================================
// Component
// ============================================================================

/**
 * SingleFormSidebar - Tabless sidebar with actions, slug, sidebar fields, SEO and document info
 */
export function SingleFormSidebar({
  document,
  _schema,
  seoField,
  actions,
  fields = [],
}: SingleFormSidebarProps) {
  const [isDocInfoOpen, setIsDocInfoOpen] = useState(true);
  const { formatDate } = useAdminDateFormatter();

  const createdAt =
    (document?.createdAt as string | undefined) ??
    (document?.created_at as string | undefined) ??
    undefined;
  const updatedAt =
    (document?.updatedAt as string | undefined) ??
    (document?.updated_at as string | undefined) ??
    undefined;

  const formatDocumentDate = (dateString: string | undefined) => {
    if (!dateString) return "—";
    return formatDate(
      dateString,
      {
        dateStyle: "medium",
        timeStyle: "short",
      },
      dateString
    );
  };

  return (
    <div className="h-full flex flex-col bg-background overflow-y-auto">
      {/* Action Buttons */}
      {actions && (
        <div className="px-6 py-4  border-b border-primary/5 bg-background shrink-0">
          {actions}
        </div>
      )}

      {/* Main content */}
      <div className="flex-1 flex flex-col pb-6">
        {/* Sidebar-positioned component fields — accordion, full-bleed */}
        {fields.length > 0 && (
          <div className="flex flex-col">
            <EntryFormContent fields={fields} className="space-y-0" />
          </div>
        )}

        {/* SEO section */}
        {seoField && (
          <div className="px-6 py-4  border-b border-primary/5">
            <EntryFormContent fields={[seoField]} />
          </div>
        )}

        {/* Document Info */}
        <div>
          <div
            onClick={() => setIsDocInfoOpen(!isDocInfoOpen)}
            className="w-full flex items-center justify-between bg-primary/5 text-primary px-6 py-3 transition-all duration-200 cursor-pointer  border-y border-primary/5 border-primary/5 hover:border-primary/50 relative z-10"
          >
            <h3 className="text-sm font-medium">Document Info</h3>
            <ChevronDown
              className={cn(
                "h-4 w-4 transition-transform duration-200",
                isDocInfoOpen ? "rotate-0" : "-rotate-90"
              )}
            />
          </div>

          {isDocInfoOpen && (
            <div className="space-y-4 px-6 pt-6 pb-6  border-b border-primary/5 border-primary/5 z-0">
              <div>
                <dt className="text-xs font-medium text-primary/50 mb-1">
                  Document ID
                </dt>
                <dd className="text-sm font-mono text-foreground break-all">
                  {document.id}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-medium text-primary/50 mb-1">
                  Created
                </dt>
                <dd className="text-sm text-foreground">
                  {formatDocumentDate(createdAt)}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-medium text-primary/50 mb-1">
                  Last Updated
                </dt>
                <dd className="text-sm text-foreground">
                  {formatDocumentDate(updatedAt)}
                </dd>
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}

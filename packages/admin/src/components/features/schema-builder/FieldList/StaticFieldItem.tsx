...
export function StaticFieldItem({
  ...
  return (
    <div
      ...
      className={`
        group flex items-center gap-3 p-2 rounded-none  border border-primary/5 transition-all
        ${isSystem ? "bg-primary/5 border-primary/5 opacity-60 cursor-not-allowed" : isSelected ? "border-primary bg-primary/5" : "border-primary/5 bg-background hover-subtle-row"}
    ...
    >
      ...
      >
        {/* Field type icon */}
        <div
          className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-none]  border border-primary/5 transition-colors ${isSystem ? "border-primary/5-foreground/20 bg-primary/5 text-muted-foreground/60" : "border-primary/25 bg-primary/5 text-primary dark:bg-primary/20 dark:text-primary-foreground/80"}`}
        >

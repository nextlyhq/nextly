...
const booleanRenderer: CellRenderer = ({ value }: CellContext) => {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-sm px-2 py-0.5 text-[11px] font-medium border border-border",
...
const badgeRenderer: CellRenderer = ({ value }: CellContext) => {
  if (value === null || value === undefined || value === "")
    return <EmptyValue />;
  return (
    <span className="inline-flex items-center rounded-sm px-2 py-0.5 text-[11px] font-medium bg-muted text-foreground border border-border">
      {toText(value)}
    </span>
  );
};
  ...
  return (
    <span className="flex flex-wrap items-center gap-1">
      {shown.map((v, i) => (
        <span
          key={i}
          className="inline-flex items-center rounded-sm px-1.5 py-0.5 text-[11px] font-medium bg-muted text-foreground border border-border"
        >
          {toText(v)}
        </span>
      ))}

...
function assertFitsCaps(
  ...
  // Unstorable and too large are different verdicts, and saying so is the point.
  ...
  if (size.unserializable) {
    throw new OpError(
      `${verb}: this would leave the document holding a value that cannot be ` +
        `stored — a cycle, or something JSON does not preserve. The edit ` +

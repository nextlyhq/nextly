 ...
 * ignores {@link InlineCommitRefused} does not compile, which is the only
 * version of this that stays true as surfaces are added.
 *
 * @module inline-commit
 */

import type { BlockDocument } from "@nextlyhq/blocks-engine";

/** Why a finished edit could not be written. */
export type InlineCommitRefusal =
  /**
 ...
 *
 * Distinct from {@link InlineCommitRefused} in the one way that matters to a
 * host: there is nothing left to protect, so this must not stop it closing.
 */
export interface InlineCommitDiscarded {
 ...
 */
export interface InlineCommitRefused {
  readonly status: "refused";
  readonly reason: InlineCommitRefusal;
}

/** The outcome of finishing an inline edit. */
...
export const INLINE_COMMIT_UNCHANGED: InlineCommitUnchanged = {
  status: "unchanged",
};

/** The author's typing could not be stored and the surface has been released. */
export const INLINE_COMMIT_DISCARDED: InlineCommitDiscarded = {
  status: "discarded",
};

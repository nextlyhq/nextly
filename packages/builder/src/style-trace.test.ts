...
describe("the cascade fetched for the panel", () => {
  ...
  it("keeps a child stored under a slot the definition does not declare", () => {
    /*
     * The tree the trace is compiled from, asserted against the tree the render
     * actually draws — which are not the same tree, and the gap is one pass.
     *
     * `renderSlot` reads the stored slot by name and never consults the
     * resolver, so a block whose definition dropped a slot, or a third-party one
     * rendering a name it never declared, puts those children on the page. The
     * preparation pipeline's last pass assumes the opposite and removes them.
     * Compiled from that stage, the panel reported every control on a visibly
     * styled block as set by nobody.
     *
     * The RENDER half is asserted first and separately, because it is what makes
     * the second half a defect rather than a preference: without it, an empty
     * trace would be the correct answer for markup nobody receives.
     */
    register();
    registerBlocks(
      [
        {
          name: "acme/legacy",
          version: 1,
          description: "Declares no slots and renders a stored one anyway.",
          example: { props: {} },
    ...
    const trace = pageStyleTrace(page, { breakpoints: BREAKPOINTS }, undefined);

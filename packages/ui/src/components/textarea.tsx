...
>(({ className, ...props }, ref) => {
  return (
    <textarea
      data-slot="textarea"
      className={cn(
        "flex min-h-[80px] w-full rounded-none border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground placeholder:opacity-50 transition-all duration-200 focus:border-ring focus:outline-none hover:border-border disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-primary/5 resize-y",

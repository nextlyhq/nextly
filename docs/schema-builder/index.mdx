...
After save, the new collection appears in the admin sidebar under Collections. The Drizzle migration is written to your `db.migrationsDir` and should be committed alongside your code.
...
| Single | `dynamic_singles` table + the `ui-schema.json` manifest | No migration file. The schema is regenerated from the database row on each request; the manifest entry is what `migrate:create` reads to carry the entity into a migration. |
| Component | Database row + the `ui-schema.json` manifest | No migration file. Stored alongside dynamic singles; the manifest entry is what `migrate:create` reads. |
...
An edit that changes only **registry metadata** — a label, a description, or the Versions, Revalidate or Webhooks switches — changes no column, so no migration file is written for it on its own. It reaches other environments through the committed `ui-schema.json` manifest instead: see [Builder entities and production](/docs/guides/production-migrations#builder-entities-and-production).

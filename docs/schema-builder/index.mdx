...
The **Visual Schema Builder** is Nextly's drag-and-drop schema designer. It produces the same registered Collections, Singles, and Field Groups that the code-first approach produces, plus the matching TypeScript types. Only the Collections Builder writes a file to disk: a Drizzle migration. Singles and Field Groups persist to the database alone — see [Persistence](#persistence). You can switch between the Visual Schema Builder and code-first definitions at any time — they are interchangeable per entity.

The Schema Builder has three variants that share the same canvas, palette, and field editor:

| Variant | Route | Output |
|---|---|---|
| Collections Builder | `/admin/builder/collections` | Persists the schema row and writes a Drizzle migration under `db.migrationsDir` (default `./src/db/migrations/`). No TypeScript schema file is written: the runtime builds its Drizzle table from the `dynamic_collections` row instead. |
| Singles Builder | `/admin/builder/singles` | Persists the schema in the `dynamic_singles` database table. No files written to disk. |
| Field Groups Builder | `/admin/builder/field-groups` | Persists the field group definition in the database. No files written to disk. |

...
An edit that changes only metadata — a label, a description, or the Status, Internationalization, Versions, Revalidate or Webhooks switches — changes no column, so no migration file is written. That edit applies to the database the running process is connected to and does not travel to another environment on its own. Plan for it: see [Builder entities and production](/docs/guides/production-migrations#builder-entities-and-production).

Applied migrations are not re-run: `nextly migrate` records every file it applies in the `nextly_migrations` table and skips what is already there. That ledger belongs to the migration runner, not to the Builder's file writer.

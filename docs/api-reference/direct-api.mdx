...
> **Note:** Direct API methods are currently named `findGlobal` / `updateGlobal` / `findGlobals`. They will be renamed to `findSingle` / `updateSingle` / `findSingles` in a future release. The prose in this guide says "Single", but the method names match what is in the runtime today.
...
const settingsLike = await nextly.findGlobals({ search: "settings" });
```
...
**Returns:** `SingleListResult` — `{ docs: GlobalEntry[], totalDocs, limit, offset }`. Each `GlobalEntry` is `{ slug, label, data }`.

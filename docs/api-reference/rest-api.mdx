...
curl https://example.com/api/collections/posts/entries \
  -H "Authorization: Bearer nx_live_<random>..."
```
...
| `GET` | `/api/field-groups` | List field group definitions. | Authenticated |
| `POST` | `/api/field-groups` | Create field group. | `manage-settings` |
| `GET` | `/api/field-groups/{slug}` | Get field group. | Authenticated |
| `PATCH` | `/api/field-groups/{slug}` | Update field group. | `manage-settings` |
| `DELETE` | `/api/field-groups/{slug}` | Delete field group. | `manage-settings` |
| `POST` | `/api/field-groups/schema/{slug}/preview` | Dry-run field group schema diff. | `manage-settings` |
| `POST` | `/api/field-groups/schema/{slug}/apply` | Apply field group schema changes. | `manage-settings` |
...
A plugin may serve its own endpoints, and they are not listed here. `/api/forms`, `/api/forms/{slug}` and `/api/forms/{slug}/submit` are served by [`@nextlyhq/plugin-form-builder`](/docs/plugins/form-builder) at those exact addresses, and answer only when it is installed. They were once built in, which never worked without the plugin either: the collections they read are declared by it.

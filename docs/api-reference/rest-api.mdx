...
> **Note:** Direct API methods are currently named `findGlobal` / `updateGlobal` / `findGlobals`. They will be renamed to `findSingle` / `updateSingle` / `findSingles` in a future release. The REST surface already uses `/api/singles/...` everywhere — only the in-process method names lag.

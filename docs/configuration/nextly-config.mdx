...
| `fieldGroups` | `FieldGroupConfig[]` | `[]` | Reusable field groups. See [Field Groups](/docs/configuration/components). |
...
Array of reusable field-group templates created with `defineFieldGroup()`. Components are embedded inside collections, singles, or other components via the `component` field type. Slugs must be unique across collections, singles, and components.

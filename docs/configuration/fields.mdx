...
Every handler receives its own `value` **and the whole record as `data`**. A
`beforeChange` handler can therefore read, and change, fields other than its
own, and because it is the last thing to run before the write, nothing after it
will put them back. Return the value you want stored; touch other fields only

...
- **Password fields** are stripped from every read before `afterRead`, on
  collections and Singles alike, so a password field's `afterRead` does not
  run. A collection- or Single-level `afterRead` hook that puts the key back
  runs before the field hooks, so then it does, and the response is stripped
  again afterwards.
...
- **Localized fields** live in the companion table and are merged into the
  ...
  The working-draft update described under field groups is the exception
  here too: its assembled draft carries the locale's full companion values,
  unchanged fields included, so their hooks run.

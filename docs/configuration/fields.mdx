...
do. The one write where it does run is a working-draft update: on a collection
with `versions: { drafts: true }`, an update to a published entry that names no
`status` is held as a draft, and `afterChange` runs over that assembled draft,
field groups included. A value returned from `afterChange` reshapes the
response and does not change what was stored.

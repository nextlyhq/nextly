...
Each phase runs **after** the collection-level hook of the same name; see the
[execution order](/docs/configuration/collections#execution-order) for how the
two levels interleave.
...
Every handler receives its own `value` and, as `data`, **the record at its own
nesting level**: the whole document for a top-level field, the containing row
for a field inside a group or a repeater. A `beforeChange` handler can read and
change sibling fields at that level, and no later hook will put them back.
Return the value you want stored; touch siblings only when that is the intent.
...
normalises the record after the last hook: slugs are re-sanitised, passwords
hashed, relationship and upload values reduced to ids. See the

...
| `virtual` | `boolean` | `false` | On a collection, Single or component: store nothing. The field gets no column, a value sent for it on create or update is dropped before the write, and it is never localized. Compute its value in an `afterRead` hook. Group and repeater also accept `options.virtual`. Custom user fields are always stored, so user config validation refuses `virtual` on them. |
...
create that means the request with field defaults applied before any hook
runs, constant and function alike (see [`defaultValue`](#defaultvalue)), so a
defaulted field's hook runs on a value nobody submitted, and a
...
Both forms are applied on the server, so a default holds for every write
path that applies defaults, not only for the admin form. On an ordinary
collection create through the REST or Direct API, an omitted field takes its
default before any hook runs, at every depth: a group's children and each
supplied repeater row's children. A new field-group instance also fills its own
children's defaults, but LATER: a field group's rows are written in their own
pass after the entry's hooks have run, so the parent's hooks see such a child as
absent and cannot read or derive from the value that will be stored. A function
default is read from the live config, since a
collection's stored definition cannot hold a function, and it receives the
data built so far at its own level: the document for a top-level field, the
group or the row for a nested one. Rows are never invented; `minRows` is a
validation rule, not an instruction to create rows.

A field-group child takes its default the same way, constant or function. Its
fields are read from a stored definition, which cannot carry a function, so the
function form is read from the live config exactly as a collection's is. It
resolves against the instance being built, so a child may compute from a sibling
that was defaulted before it.

A Single's first read creates its row with the same defaults, top-level and
nested, with two exceptions: a default on a `fieldGroup` field itself is
evaluated and then dropped because that field has no column on the row, and a
password field's default, at any depth, is refused because that path never
hashes.

Bulk creates (`createMany`) and creates inside a transaction apply no defaults
to the entry's own fields; a new field-group instance on those writes still
takes its own defaults.

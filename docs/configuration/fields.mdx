...
Inside a reusable field group, only a constant default applies. A field
group's fields are read from its stored definition on every write, and a
function cannot be stored, so a `defaultValue` written as a function on a
field-group child is not applied anywhere: not on a collection create, not on
a Single's first read, and not in the admin form. Give a field-group child a
constant default, or set the value from the parent's own field.

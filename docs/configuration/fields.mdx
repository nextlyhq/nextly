...
when the patch carried it, so an unchanged localized field's `afterChange`
does not run on an update. On a collection read, field-level `access.read`
...
through the REST or Direct API, a constant default is applied before any hook
runs; a function default is not, because a collection's fields reach that

...
the row, persisted or loaded, so a hooked field runs whether or not it changed.
Two qualifications. On a localized collection or Single, a translated field
lives in the companion table and is merged into the `afterChange` row only
when the write carried it, so a localized field omitted from a create, or
unchanged on an update, has no `afterChange` run. On a collection read, field-level `access.read`
rules are applied before the hooks, so a field the caller may not read is gone
from the row by then and its `afterRead` does not run; a Single read runs the
hooks first and redacts afterwards, so there a denied field's `afterRead` does
run and sees the value, and only the response is redacted. One exception, on every write to a collection or a Single: a `fieldGroup` field
is stored in its own table and is removed from the row before the main-table
write, and the `afterChange` hooks run on the returned row without it, so that
field's `afterChange` hook does not run. Its `beforeValidate` and `beforeChange`
do. The one write where it does run is a working-draft update through the
ordinary update path: on a collection with `versions: { drafts: true }`, an
update to a published entry that names no `status` is held as a draft, and
`afterChange` runs over that assembled draft, field groups included. A
collection is not eligible to hold drafts when it or one of its field groups
has a password field, or when a field group's schema cannot be resolved; a
status-less update there writes the live row and the field group's
`afterChange` is skipped as on any other write. An update
made inside a caller's transaction runs `afterChange` over the main row and
skips the field group there too. A value returned from `afterChange` reshapes
the response and does not change what was stored.
...
both forms for its top-level fields (a default on a field nested inside a
`group` is not resolved there), except for a password field, whose default of
either form is refused because that path never hashes. Bulk creates (`createMany`) and

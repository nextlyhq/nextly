...
One exception, on every write to a collection or a Single: a `fieldGroup` field
...
`afterChange` runs over that assembled draft, field groups included. An update
...
so an omitted field with a function default is written without a value. A
Single's first-read auto-create holds the code config directly and resolves
both forms. Bulk creates (`createMany`) and creates inside a transaction apply
no defaults of either form.

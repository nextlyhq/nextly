...
as transformed so far, not a copy of what the caller sent. On a create that
means the request with field defaults applied and a `slug` and title generated
before the first hook runs, so a hook can see, and a defaulted field's hook can
run on, a value nobody submitted. On an update it is the patch, not the stored
row, so an unchanged sibling reads as `undefined`. Do not read a key's presence
as proof the caller supplied it. A `beforeChange` handler can read and change
...
One exception on a collection create: a `fieldGroup` field is stored in its own
table and is removed from the row before the insert, and the `afterChange`
hooks run on the returned row without it, so that field's `afterChange` hook
does not run there. A value returned from `afterChange` reshapes the response
and does not change what was stored.

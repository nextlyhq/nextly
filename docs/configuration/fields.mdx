...
**Before the write** (`beforeValidate`, `beforeChange`), `data` is what was
submitted: the document on a create, the patch on an update, not the stored
row, so an unchanged sibling reads as `undefined`. A `beforeChange` handler
can read and change siblings at that level. No collection-level phase runs
after it, but a sibling's own hook registered later in the same phase still
can. Return the value you want stored; touch siblings only when that is the
intent. These hooks run only when their field is present in the data: an
update that omits it, or an earlier hook that deletes the key, skips them.

**After the write** (`afterChange`) and **on a read** (`afterRead`), `data` is
the full row, persisted or loaded, so every hooked field runs whether or not it
changed. A value returned from `afterChange` reshapes the response and does not

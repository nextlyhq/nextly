...
collection hook in that phase cannot. A bulk create (`createMany`) and a create
...
On a read, field-level `access.read` rules are applied before the hooks, so a
field the caller may not read is gone from the row by then and its `afterRead`
does not run. One exception, on every write to a collection or a Single: a `fieldGroup` field
...
both forms, except for a password field, whose default of either form is
refused there because that path never hashes. Bulk creates (`createMany`) and

...
- **Denied fields.** On a collection read, field-level `access.read` rules are
  applied before the hooks, so a field the caller may not read is gone by then.
  A Single read currently runs the hooks first and redacts afterwards, so there
  a denied field's `afterRead` does run and sees the value, and only the
...
default before any hook runs, at every depth: a group's children, each
supplied repeater row's children, and the children of each new field-group
instance. A function default is read from the live config, since a

...
| `defaultValue` | `ArrayRowValue[] \| (data) => ArrayRowValue[]` | — | Static default rows. |
...
| `admin.components.RowLabel` | `React.ComponentType<ArrayRowLabelProps>` | — | Custom row-label component. |
...
> **Note:** Repeater is the renamed version of what some other CMSes call "array". The internal `type` is `"repeater"`. Both `repeater()` and `array()` helper functions exist in code today, but the `array()` helper is being removed; use `repeater()`.

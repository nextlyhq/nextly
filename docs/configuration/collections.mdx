...
<Callout type="warn">
  **The field-level `beforeChange` is the last hook before the write.** A
  ...
  - **`data` is what was submitted, at the field's own level.** On a create it
    is the submitted document; on an update it is the submitted patch, not the
    stored row, so an unchanged sibling reads as `undefined`. For a field inside
    a group or a repeater it is the containing row, not the top-level document.
</Callout>

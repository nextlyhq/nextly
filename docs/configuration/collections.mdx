...
<Warning>
  **The field-level `beforeChange` is the last hook before the write, and it
  receives the whole record.** A field hook is handed `data` alongside its own
  `value`, so it can change any field, not only its own. That means a
  collection-level `beforeChange` is not the final word: a field hook
  registered later, by a plugin or by the application, runs after it and can
  overwrite what it set.

  There is no collection-level phase after the field hooks. If you need a
  guarantee about the stored shape of a record, put it in the field hook of the
  field it concerns, which is the latest point that runs. This matches

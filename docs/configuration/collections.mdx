...
Reads run `beforeRead`, the query, then `afterRead` at the collection level and
then per field. Deletes run `beforeDelete` and `afterDelete`.
...
<Callout type="warn">
  **The field-level `beforeChange` is the last hook before the write.** A
  ...
  Three things bound what a field hook can guarantee:

  - **It is the last hook, not the last step.** After every hook has run, core
    still normalises the record before the write: a `slug` is re-sanitised, so
    a hook returning `"Changed By Hook"` stores `changed-by-hook`; password
    fields are hashed; an explicit `status: undefined` is dropped; relationship
    and upload values are reduced to ids. A hook whose output must survive
    exactly cannot rely on being last.
  ...
  - **A nested field's hook sees its own row.** For a field inside a group or a
    repeater, `data` is the containing row at that nesting level, not the
    top-level document.
</Callout>

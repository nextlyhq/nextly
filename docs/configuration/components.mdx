...
> **Source of truth:** the `FieldGroupConfig` interface lives in `packages/nextly/src/components/config/types.ts`. The `defineFieldGroup()` helper is in `packages/nextly/src/components/config/define-field-group.ts`.

Key characteristics:

- **Templates, not documents** — components define a field structure; each embed creates its own data row.
- **Own database table** — each component type gets a table derived from its slug and prefixed with `comp_` (e.g. `comp_seo`). The name is always derived; it cannot be overridden.

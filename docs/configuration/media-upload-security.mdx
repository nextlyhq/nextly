...
| `@nextlyhq/storage-vercel-blob`     | **Refuses SVG and HTML uploads** entirely with a `NextlyError.validation` (code `UNSUPPORTED_FOR_BACKEND`). The platform cannot reliably serve attachment-disposition. Use S3/R2, or convert to a raster format. |

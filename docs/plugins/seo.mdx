...
sitemap that is quietly short. A corpus that large needs a sitemap **index** splitting it
into several documents, which belongs to the delivery layer: Next.js `generateSitemaps()`
is the usual answer, using the exported generators below for each shard.

...
Nextly plugins expose HTTP endpoints via [`contributes.routes`](/docs/plugins/routes) -- the route handler receives the plugin's `ctx` for free, which is what `submitForm` needs as `pluginContext`.
...
// Both values belong to the form-builder plugin's own context. There is no
// supported way to reach them from a plain Next.js route handler: the resolved
// config is stored on the Nextly instance during the plugin's init, and
// `getFormBuilderConfig` is not reachable from another plugin's context. Call
// `submitForm` from inside the form-builder's own seam, or use the route the
// plugin contributes, rather than rebuilding this by hand.
declare const pluginContext: PluginContext;
declare const pluginConfig: ResolvedFormBuilderConfig;

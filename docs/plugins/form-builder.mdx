...
Nextly plugins expose HTTP endpoints via [`contributes.routes`](/docs/plugins/routes), and a contributed route receives the plugin's `ctx`, which is what `submitForm` takes as `pluginContext`.

`pluginConfig` is the harder half, and worth saying plainly before you copy anything: the form-builder resolves its config during its own `init` and stores it on the Nextly instance, and no exported accessor reaches it from another plugin's context. So the snippet below is the **shape** `submitForm` expects, not a route you can paste and run — the two context values are declared rather than obtained, because today there is no supported way to obtain them outside the plugin itself.
...
// Declared, not obtained: see the note above. `declare` types these for the
// example and produces nothing at runtime, so this file will not run as
// written.
declare const pluginContext: PluginContext;
declare const pluginConfig: ResolvedFormBuilderConfig;
...
Add `contactRoutePlugin` to `plugins` alongside `fb.plugin`, and the form now posts to `/api/plugins/@mysite/contact-route/submit`.

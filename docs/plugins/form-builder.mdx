...
import { submitForm, getFormBuilderConfig } from '@nextlyhq/plugin-form-builder';

export const contactRoutePlugin = definePlugin({
  name: '@mysite/contact-route',
  version: '1.0.0',
  nextly: '^1',
  contributes: {
    routes: [
      {
        method: 'POST',
        path: '/submit',
        public: true, // anonymous visitors submit the contact form
        handler: async (req, ctx) => {
          const data = await req.json();

          const pluginConfig = getFormBuilderConfig(ctx);
          if (!pluginConfig) {
            return Response.json(
              { success: false, error: 'Form Builder plugin not installed' },
              { status: 500 }
            );
          }
          ...
          const result = await submitForm(
            ...
            { pluginContext: ctx, pluginConfig }
          );

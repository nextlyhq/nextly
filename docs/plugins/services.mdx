...
A code that is not a configured locale resolves to the default rather than failing, the
same way `?locale=` does on the REST API. The pair is spelled exactly as it is on the
request context and on the wire, so a route can pass through what it was given.

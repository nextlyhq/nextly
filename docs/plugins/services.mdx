...
ask the core which spellings are selectors — `isLocaleSelector` from `nextly` — rather
than keep a list of its own. The same spellings are reserved at configuration, so a site
cannot name a language `all`.

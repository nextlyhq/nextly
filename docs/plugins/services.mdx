...
It is the live Drizzle instance for the configured dialect, so use its fluent
API (`select`, `insert`, `update`, `delete`). There is no relational `query`
namespace: the one that used to exist named a few core tables, so it could
never answer about a plugin's own tables.

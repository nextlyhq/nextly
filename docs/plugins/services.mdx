...
| `opts` | Behaviour |
|---|---|
| `{ as: "user", user, authenticatedScope }` | Runs as that caller — **RBAC enforced**. Pass the route's `ctx.user` AND `ctx.authenticatedScope`: `user` names the account, which for an API key is the key's OWNER, so forwarding it alone authorizes the key as whoever minted it. |
...
records should use `{ as: "user", user: ctx.user ?? undefined, authenticatedScope: ctx.authenticatedScope }`.

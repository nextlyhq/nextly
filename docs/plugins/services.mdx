...
```ts
const { data: rows } = await ctx.services.collections.listEntries(
  slug,
  { where: { status: "draft" }, pagination: { limit: 100 }, depth: 0 },
  { as: "system" }
);
await ctx.services.collections.updateMany(
  slug,
  rows.map(row => ({ id: row.id as string, data: { status: "published" } })),
  { as: "system" }
);
```

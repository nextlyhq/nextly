...
| `PLUGIN_MIGRATIONS_UNAVAILABLE` | 409 | `nextly migrate` found tables owned by a plugin that ships no migration modules, so production has no way to create them. | Generate the plugin's modules with `nextly migrate:create --plugin <name>`. |

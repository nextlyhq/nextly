...
### Forward the scope, not just the user

`ctx.user` names the ACCOUNT. When the request arrived on an API key that account is
the key's **owner**, so a service asked to judge `user.id` alone resolves the owner's
roles — and a key scoped to read is then allowed to do whatever its owner can. Pass
`ctx.authenticatedScope` alongside it and the key is judged on its own grants:

```ts
const posts = await ctx.services.collections.listEntries("posts", {}, {
  as: "user",
  user: ctx.user ?? undefined,
  // Without this a read-only key minted by an administrator is authorized as
  // that administrator.
  authenticatedScope: ctx.authenticatedScope,
});
```

A session caller carries no scope, so passing it changes nothing for them — it is
`undefined`, and they resolve exactly as they always have.

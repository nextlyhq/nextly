...
`ctx.authenticatedScope` carries the same grants twice, because a permission is
written two ways and both are correct:

| Field | Spelling | Where it is used |
|---|---|---|
| `permissions` | `read-posts` | The stored slug — the database, the admin's permission matrix, and `requiredPermission` on a route. |
| `rulePermissions` | `posts:read` | What a code-defined `access` rule receives, on a collection, a Single or a field. |

Both are built from the same permission row, so they always name the same set.
Read `permissions` when you are comparing against a slug; read `rulePermissions`
when you are reproducing what an `access` rule would see.

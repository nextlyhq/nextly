...
- **`contributes.admin`** — the _declarative_ surface: menu entries, custom pages,
  a settings panel, and per-collection view overrides (D19–D23).
- **`admin`** (on the plugin definition) — _placement & appearance_: where the
...
a type error. Every other archetype requires it.
...
An archetype this version of Nextly does not draw yet reports itself in that card and
leaves the rest of the dashboard alone, so a plugin built against a newer core degrades
one card rather than breaking the admin.

The contract is still `@experimental` and is expected to grow: charts, a per-instance
config schema, and saved layouts. Pin your Nextly version while it settles.

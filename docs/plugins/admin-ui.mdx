...
export function ChartWidget({ slot, settings, isFetching }: WidgetComponentProps) {
  if (!slot) return null;                 // this widget declared no query
...
| `slot` | `WidgetSlot \| undefined` | This card's answer, or `undefined` when the widget declared no query. |
| `isFetching` | `boolean` | Whether a refresh is in flight. |

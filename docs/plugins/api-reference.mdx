...
config-time rule that needed I/O would make boot depend on something that can be
down. Issue paths are **relative** and get appended to the field's own, so
...
**Where it runs.** Code-first collection, single, and component configs, and
Schema Builder saves. One caveat on the Builder path: the stored manifest
declares its option keys and strips the rest, so a plugin type's own options do
not currently survive a Builder save and there is nothing there for the check to
read. Options the manifest already declares are checked normally.

...
Each is present only when the plugin declares the matching contribution, so a
plugin that never asked for a surface does not have one.
...
[security](/docs/plugins/security) for what is and is not returned to the admin.

### `ctx.fetch`

Declare `capabilities.net.outbound` to get it.
...
Kinds must start with the plugin's own admin slug. Undeclared kinds and keys
...
Also `ctx.auth.currentUser(request)` and `ctx.auth.verifyCsrf(request)`.
...
`onInstall` and `onUninstall` are declared for the install and uninstall
commands — a boot never calls them. The commands that invoke them arrive with
the plugin migration work, so today nothing calls them: treat them as the
contract for that flow, not as a running lifecycle.

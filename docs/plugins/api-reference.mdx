...
interface PluginFieldType {
  ...
  layout?: "takeover";
  validate?: (value, args) => true | string | PluginFieldIssue[] | Promise<…>;
}
```
...
fieldTypes: [
  {
    ...
    validate(value, { field }) {
      ...
      if ((value as number) > max) return `Rating cannot exceed ${max}`;
      return true;
    },
  },
]
...
`args` carries `data` (the whole entry, for rules spanning fields), `req`
(carries `user` when authenticated), `field` (the instance, so you can read the
options your type declares), and `mode` (`"create"` | `"update"`).

Return `true` to accept, a string for one problem, or an array when one value
can be wrong in several places at once:

```ts
return [
  { path: "content.nodes[2]", code: "DISALLOWED", message: "That block is not allowed here" },
  { message: "Document exceeds the node limit" },   // path/code default to the field's own
];
```

Ordering: built-in rules for the storage primitive run first, then this, then
the field's own `validate` — so a schema author adds rules on top of yours
rather than replacing them. An absent value never reaches `validate`; that is
what `required` is for. Throwing is treated as a refusal, not a server error.

...
**Where it runs.** Wherever a config is validated — `defineCollection`,
`defineSingle`, `defineComponent`, and Schema Builder saves. Two caveats worth
knowing:

- A `define*` call validates when its module is evaluated, so your type must
  already be registered by then. This is not a new constraint: an unregistered
  type is refused outright as an unknown field type. Boot does not re-validate
  configs after registering field types, so a config that was never validated is
  not validated later either.
- The stored manifest declares its option keys and strips the rest, so a plugin
  type's own options do not currently survive a Schema Builder save and there is
  nothing there for the check to read. Options the manifest already declares are
  checked normally.

...
- [Lifecycle, dependencies & order](/docs/plugins/lifecycle) — `setup`/`init`/`destroy`, load order, `dependsOn`, version compat, `enabled`
...
// "sqlite" is always available; the others need TEST_POSTGRES_URL /
// TEST_MYSQL_URL to point at a server you are willing to create databases on.
for (const dialect of getAvailableTestDialects()) {

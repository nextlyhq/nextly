...
**Prerequisites:** Complete the [Installation](/docs/getting-started/installation) steps first, including creating the super admin at `/admin/setup`.
...
> **Note:** The Direct API single methods are currently named `findGlobal` / `updateGlobal` / `findGlobals`. They will be renamed to `findSingle` / `updateSingle` / `findSingles` in a future release. Update your imports when that change ships.
...
- [Project Structure](/docs/getting-started/project-structure) -- Understand how the project is organised

...
> **Minimum version: SQLite 3.38+.** The bundled `better-sqlite3` ships with a newer SQLite by default, so production users on the pinned package always pass the version check. See [database support](./support) for the full version policy.

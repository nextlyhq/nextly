...
| [Neon](https://neon.tech) | Supported | Auto-detected from URL pattern (`*.neon.tech`). Cold-start retries built into the adapter. SSL on by default. Vercel projects also reach Postgres through Neon since the [Vercel Postgres -> Neon migration](https://neon.com/docs/guides/vercel-postgres-transition-guide) in late 2024. |
...
declare const adapter: import("@nextlyhq/adapter-drizzle").DrizzleAdapter;

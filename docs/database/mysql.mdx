...
> **Minimum version: MySQL 8.0+.** MySQL-compatible variants (MariaDB, TiDB, Aurora MySQL, PlanetScale, Vitess) are recognized at connect with a warning and continue to work on best-effort support. See [database support](./support) for variant policy and behavior details.

...
Installs that never run the drain — for example, a cron-driven or serverless deploy where no long-lived process fans out deliveries — should trigger a retention pass on a schedule.
...
The command reports how many webhook events, audit events, and terminal delivery rows it removed. If a batch bound stops the pass before the queue is drained, it says so — run it again to continue. It reads the same `webhooks.retention` policy as the automatic passes, and does nothing when retention is disabled.

A daily cron is a good baseline:

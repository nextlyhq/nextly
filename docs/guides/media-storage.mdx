...
`getMediaVariant` is what to use in both cases. It asks for a variant by name, tries
your fallback name next, then the thumbnail, then the original, so the same component
works whether or not variants reached it:

```tsx
// app/(site)/photo/page.tsx
import Image from "next/image";
import { getMediaVariant, getNextly } from "nextly";
...
export default async function Photo() {
  ...
  return (
    <Image
      src={getMediaVariant(media, "large") ?? media.url}
...
`getNextly({ config })` initialises the runtime if nothing has yet, so this page works
wherever it renders. The synchronous proxy from `nextly/runtime` assumes an already
initialised process and throws `Nextly services not initialized` when it is the first
Nextly code a worker runs, which a blank scaffold makes likely because it ships no
`instrumentation.ts`.
...
The component takes the two values rather than a whole media record, because
`media.findByID()` does not return them today: `MediaFile` has no `focalX` or `focalY`,
so read the crop point from the media row until it does.

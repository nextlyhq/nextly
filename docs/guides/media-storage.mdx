...
import { getMediaVariant, type MediaLike } from "nextly";

export function MediaImage({ media, alt }: { media: MediaLike; alt: string }) {
  return (
    <Image
      src={getMediaVariant(media, "large", { fallback: "medium" }) ?? media.url}
      alt={alt}
      width={1200}
      height={800}
    />

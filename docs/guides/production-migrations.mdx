...
A plugin migration carries a checksum over its own SQL, and the ledger
separately stores the sha256 of what actually ran.

Neither covers the other. The first catches a module edited after generation —
the SQL that will run is not the SQL that was reviewed. The second catches a
module changed after it was applied, where the database no longer matches the
file claiming to describe it. A regenerated module with a rewritten checksum
passes the first and fails the second, which is exactly right.

...
and admin display settings. Collections and Singles additionally carry the Versions, Revalidate and
Webhooks switches; Field Groups have none of those, and `dynamic_components` has no columns for
them. The runtime reads its Drizzle table from that row, so the row matters as much as the columns
do.

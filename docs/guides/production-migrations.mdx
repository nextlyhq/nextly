...
three entity kinds. Commit it with your migrations: it is the **input** `nextly migrate:create` reads
to know a Builder entity exists, and a Builder change missing from the commit cannot appear in any
migration.
...
After applying files, `reconcileMigrationMetadata` calls `registerFromMigrations`, then checks
pending entities against their physical tables. It is narrower than it sounds, so expect exactly
this and no more:

| Case | Repaired by `nextly migrate`? |
|---|---|
| A missing **collection** or **single** registry row, where an applied migration's snapshot describes it | Yes — the row is inserted |
| A row whose `migration_status` is stale for a table that does exist | Yes — the status is advanced or reported |
| A missing **field group** row | No — `registerFromMigrations` covers collections and singles only |
| An existing row with stale labels, descriptions or fields | No — the sweep updates status, it does not refresh content |
| A metadata-only edit that produced no file | No — there is no artifact for the reconciler to read |

For the first two, run `nextly migrate` rather than recreating the entity by hand.
...
Remove the entity from `ui-schema.json` as part of the move. `migrate:create` tolerates the
collision — it warns and lets the code-first definition win — but `nextly migrate:check` runs
`validateCrossFile` and fails on `NEXTLY_SCHEMA_SLUG_COLLISION`, so a conversion that leaves the
manifest entry in place generates a migration locally and is then rejected by CI.

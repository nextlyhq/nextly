...
**Setting `NEXTLY_APPLIED_BY`** is optional but recommended. It populates the `applied_by` column on the `file_apply` event in `nextly_schema_events` so post-incident debugging can answer "which CI job applied this row?" When unset, the runtime falls back to `GITHUB_ACTOR` -> `USER` -> hostname.
...
6. Records each applied file in `nextly_schema_events` as a `file_apply` event with `applied_at`, `applied_by`, `duration_ms`, `sha256`.
...
`dynamic_collections`, `dynamic_singles` or `dynamic_components` — holding the label, description,
admin display settings and the Versions, Revalidate and Webhooks switches. The runtime reads its
Drizzle table from that row, so the row matters as much as the columns do.

### Commit `ui-schema.json`

The Builder dual-writes every entity it creates or saves into the `ui-schema.json` manifest, for all
three entity kinds. `nextly migrate:create` reads that manifest and emits each entity's registry-row
upsert alongside the DDL, so **the manifest is how a Builder entity reaches another environment.**

Treat it as source: commit `ui-schema.json` with your migrations. A Builder change that is missing
from the commit is missing from production, and Singles and Field Groups have no other carrier —
they write no migration file of their own.

The manifest is written by a development-only route. It is a dev-time artifact you commit, not
something production generates.

### What still does not travel

A **metadata-only** edit — one that changes a label, a description, or the Versions, Revalidate or
Webhooks switch and touches no column anywhere in the run — produces no migration file, because
`migrate:create` diffs DDL shape and finds nothing to write (`No changes detected`, exit code 2).
The manifest records the change; nothing applies it to another database until some later run writes
a file. Note that Status and Internationalization are *not* in this category: both change columns
and produce a migration.

### What `migrate` repairs, and what it cannot see

`nextly migrate` does more than apply files. After applying, `reconcileMigrationMetadata` calls
`registerFromMigrations` to restore missing collection and Single registry rows from applied
snapshots, then checks each pending entity's physical table and advances or reports its
`migration_status`. So a **missing or stale** registry row is exactly the case the repair path
handles — run `nextly migrate` rather than recreating the entity by hand.

What it cannot see is a metadata-only edit that never produced a file: there is no artifact for the
reconciler to read, so production keeps the previous value.

### Moving a setting into code

Declaring an entity in `nextly.config.ts` does version its settings — but `defineCollection` and
`defineSingle` take a **complete** entity definition, and on a slug collision the code-first
definition wins and the `ui-schema.json` entry is shadowed. Moving a switch into code therefore
moves the **whole entity**, its fields included, out of the Builder's ownership. Do it deliberately
as a conversion, not as a way to version one toggle.
...
| `NEXTLY_APPLIED_BY`          | `migrate`  | Recorded on the `file_apply` event's `applied_by`. Falls back to `GITHUB_ACTOR` -> `USER` -> hostname. |

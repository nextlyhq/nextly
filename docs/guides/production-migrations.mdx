...
- **A deletion leaves the registry row behind.** Deleting an entity in the Builder writes a
  migration that drops its table, so the table goes. Its registry row does not: `migrate:create`

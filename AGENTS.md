...
This gate reads the DIFF, and `.github/workflows/code-hygiene.yml` runs in two jobs. Its `Changed files` job runs `audit --gate new-only` via the official `fallow-rs/fallow` action, gating on newly introduced dead code and duplication in changed files (with `apps/playground` excluded). Its `Whole repository` job scans the entire repository at the PR branch HEAD to produce a complete step summary report covering dead code, duplication, and code health.

What CI ENFORCES and where it enforces it: `Changed files` gates on introduced DEAD CODE and introduced DUPLICATION in the PR's changed files. The whole-repo job provides comprehensive visibility into repository-wide dead code, duplication, and complexity, failing only on tooling crashes.

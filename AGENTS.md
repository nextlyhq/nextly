...
What CI ENFORCES is narrower than what it reports, and the difference is a property of the tool rather than a policy choice. In the whole-repo job only DEAD CODE gates. `fallow dupes` exits 0 whatever it finds — 846 clone groups and `--fail-on-issues` both — and its JSON carries no new-versus-inherited flag to count instead; `fallow health` matches its baseline per file and category, which does not survive the two sides being different trees, so a branch touching neither still reported ~50 findings in `admin` and `plugin-form-builder`. Gating on either would be theatre. Duplication and complexity are therefore reported and not enforced: read a green run as "this change added no dead code anywhere", not as "this change added nothing".

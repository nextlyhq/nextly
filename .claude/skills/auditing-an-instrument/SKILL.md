...
git ls-remote origin refs/heads/<the pull request's branch>
...
file that was never added. The second reads the pull request's branch where it
was pushed, so it catches a commit that was never pushed, one pushed to another
branch, and a branch someone else has since moved. It reads the branch's own
ref, not GitHub's record of the pull request head (`gh pr view <N> --json
headRefOid`), because that record lags a push: `scripts/ci-verdict-evidence.mjs`
measured it a full commit behind while the ref was already current. The third
reads the pull request's diff, so it also catches a fix made in another
checkout. That diff is drawn from the same record of the head, so right after
a push it can lag the same way: ask again after a short wait before concluding
the fix did not reach the pull request.
